...
export default function ExamAttemptClient() {
  ...
  }, [attempt, current, index, queueResponse, submitting]);
    ...
    <div className="min-h-dvh bg-[#f8f7fc] pb-24 text-foreground dark:bg-[#0d0a18] lg:pb-8">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center gap-2 px-3 sm:h-16 sm:px-5">
          <button type="button" onClick={() => setLeaveDialogOpen(true)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-border bg-card text-muted-foreground hover:bg-secondary hover:text-foreground" aria-label="Leave this exam"><X className="h-4 w-4" aria-hidden="true" /></button>
          <div className="min-w-0 flex-1 text-center sm:text-left">
            <p className="truncate text-xs font-black sm:text-sm">{attempt.courseCode}</p>
            <p className="mt-0.5 text-[10px] font-bold text-muted-foreground">Question {index + 1} of {attempt.questions.length}</p>
          </div>
          <button type="button" onClick={() => setPaletteOpen(true)} className="relative grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-border bg-card text-foreground lg:hidden" aria-label={`Open question review. ${answered} of ${attempt.questions.length} answered`}><Grid3X3 className="h-4 w-4" aria-hidden="true" />{flagged > 0 ? <span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-amber-400 px-1 text-[9px] font-black text-amber-950">{flagged}</span> : null}</button>
        ...
        <div className="border-t border-border/60 px-3 py-2 sm:px-5">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
            ...
            <p className="shrink-0 text-[10px] font-bold text-muted-foreground">{answered}/{attempt.questions.length} answered</p>
          </div>
        </div>
        <div className="h-1 bg-secondary"><div className="h-full bg-emerald-500 transition-[width]" style={{ width: `${answeredPercentage}%` }} /></div>
      </header>
      ...
      <div className="mx-auto grid max-w-6xl gap-5 px-4 py-4 sm:py-5 md:px-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <main className="min-w-0">
          ...
          <section className="-mx-4 border-y border-border bg-card px-4 py-5 sm:mx-0 sm:rounded-2xl sm:border sm:p-6 md:p-7">
            <div className="flex items-center justify-between gap-3">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">Question {index + 1}</p>
              {currentResponse.flagged ? <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-[10px] font-black uppercase text-amber-800 dark:bg-amber-950/40 dark:text-amber-300"><Flag className="h-3 w-3 fill-current" aria-hidden="true" /> Review later</span> : null}
            </div>
            <h1 ref={headingRef} tabIndex={-1} className="scroll-mt-32 pt-3 text-lg font-bold leading-7 outline-none sm:text-xl sm:leading-8">{current.prompt}</h1>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              <button type="button" disabled={submitting || timeExpired} onClick={() => queueResponse(current.id, { flagged: !currentResponse.flagged })} className={cn("inline-flex min-h-11 items-center gap-2 rounded-xl border px-3.5 text-sm font-black disabled:opacity-50", currentResponse.flagged ? "border-amber-300/60 bg-amber-100/50 text-amber-800 dark:bg-amber-950/35 dark:text-amber-300" : "border-border text-muted-foreground hover:bg-secondary hover:text-foreground")}><Flag className="h-4 w-4" aria-hidden="true" /> {currentResponse.flagged ? "Remove flag" : "Review later"}</button>
              {currentResponse.selectedOptionId ? <button type="button" disabled={submitting || timeExpired} onClick={() => queueResponse(current.id, { selectedOptionId: null })} className="min-h-11 rounded-xl px-3 text-sm font-black text-muted-foreground hover:bg-secondary hover:text-foreground disabled:opacity-50">Clear answer</button> : null}
            </div>
          </section>

          <div className="mt-4 hidden items-center justify-between gap-3 lg:flex">
            <button type="button" disabled={index === 0 || submitting} onClick={() => setIndex((value) => Math.max(0, value - 1))} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-border bg-card px-4 text-sm font-black disabled:opacity-35"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> Previous</button>
            <button type="button" disabled={submitting} onClick={() => atLast ? setSubmitDialogOpen(true) : setIndex((value) => Math.min(attempt.questions.length - 1, value + 1))} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-primary px-5 text-sm font-black text-primary-foreground disabled:opacity-35">{atLast ? "Review & submit" : "Next question"} {atLast ? <Send className="h-4 w-4" aria-hidden="true" /> : <ArrowRight className="h-4 w-4" aria-hidden="true" />}</button>
          </div>
        </main>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-28 rounded-2xl border border-border bg-card p-4">
            ...
            <PaletteLegend />
            <button type="button" onClick={() => setSubmitDialogOpen(true)} disabled={submitting} className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white disabled:opacity-60"><Send className="h-4 w-4" aria-hidden="true" /> Review & submit</button>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 px-3 pb-[max(0.65rem,env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl lg:hidden" aria-label="Exam navigation">
        <div className="mx-auto grid max-w-lg grid-cols-[3.25rem_1fr] gap-2.5">
          <button type="button" disabled={index === 0 || submitting} onClick={() => setIndex((value) => Math.max(0, value - 1))} className="grid min-h-13 place-items-center rounded-xl border border-border bg-card disabled:opacity-35" aria-label="Previous question"><ArrowLeft className="h-5 w-5" aria-hidden="true" /></button>
          <button type="button" disabled={submitting} onClick={() => atLast ? setPaletteOpen(true) : setIndex((value) => Math.min(attempt.questions.length - 1, value + 1))} className="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-black text-primary-foreground disabled:opacity-35">{atLast ? "Review your answers" : currentResponse.selectedOptionId ? "Next question" : "Skip for now"}{atLast ? <Grid3X3 className="h-4 w-4" aria-hidden="true" /> : <ArrowRight className="h-4 w-4" aria-hidden="true" />}</button>
        </div>
      </nav>

      {paletteOpen ? (
        <div className="fixed inset-0 z-50 flex items-end bg-zinc-950/60 backdrop-blur-sm lg:hidden" role="dialog" aria-modal="true" aria-labelledby="question-review-title">
          <div className="max-h-[min(90dvh,760px)] w-full overflow-y-auto rounded-t-[1.75rem] border border-border bg-background px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl">
            <div className="mx-auto max-w-lg">
              <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-border" aria-hidden="true" />
              <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">Question map</p><h2 id="question-review-title" className="mt-0.5 text-2xl font-black">Check your paper</h2></div><button type="button" onClick={() => setPaletteOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-card" aria-label="Close question review"><X className="h-4 w-4" aria-hidden="true" /></button></div>
              ...
              <div className="mt-5 grid gap-2.5 sm:grid-cols-2">
                {firstUnansweredIndex >= 0 ? <button type="button" onClick={() => { setIndex(firstUnansweredIndex); setPaletteOpen(false); }} className="min-h-12 rounded-xl border border-border bg-card px-4 text-sm font-black text-primary">Go to first unanswered</button> : null}
                <button type="button" onClick={() => { setPaletteOpen(false); setSubmitDialogOpen(true); }} className={cn("inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white", firstUnansweredIndex < 0 && "sm:col-span-2")}><Send className="h-4 w-4" aria-hidden="true" /> Submit paper</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {leaveDialogOpen ? (
        <div className="fixed inset-0 z-[60] flex items-end bg-zinc-950/60 backdrop-blur-sm sm:grid sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="leave-exam-title">
          <div className="w-full rounded-t-[1.75rem] border border-border bg-background p-5 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl sm:max-w-md sm:rounded-2xl sm:p-6">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700 dark:text-amber-300">Your timer will continue</p>
            <h2 id="leave-exam-title" className="mt-1 text-2xl font-black">Leave this mock?</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">You can return from the course page, but the countdown cannot be paused. Saved answers will remain in place.</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setLeaveDialogOpen(false)} disabled={leaving} className="min-h-12 rounded-xl border border-border bg-card px-4 text-sm font-black">Stay here</button>
              <button type="button" onClick={() => void leaveAttempt()} disabled={leaving} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-sm font-black text-primary-foreground disabled:opacity-60">{leaving ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : <ArrowRight className="h-4 w-4" aria-hidden="true" />}{leaving ? "Saving…" : "Leave safely"}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {submitDialogOpen ? (
        <div className="fixed inset-0 z-[60] flex items-end bg-zinc-950/60 backdrop-blur-sm sm:grid sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="submit-exam-title">
          <div className="w-full rounded-t-[1.75rem] border border-border bg-background p-5 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl sm:max-w-md sm:rounded-2xl sm:p-6">
            <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">Final check</p><h2 id="submit-exam-title" className="mt-1 text-2xl font-black">Submit this paper?</h2></div>{!timeExpired ? <button type="button" onClick={() => setSubmitDialogOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-card" aria-label="Close submission dialog"><X className="h-4 w-4" aria-hidden="true" /></button> : null}</div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {!timeExpired ? <button type="button" onClick={() => setSubmitDialogOpen(false)} className="min-h-12 rounded-xl border border-border px-4 text-sm font-black">Keep checking</button> : null}
              <button type="button" onClick={() => void submitAttempt(timeExpired ? "timeup" : "manual")} disabled={submitting} className={cn("inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white disabled:opacity-60", timeExpired && "col-span-2")}>{submitting ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : <Send className="h-4 w-4" aria-hidden="true" />} {submitting ? "Submitting…" : timeExpired ? "Retry submission" : "Submit paper"}</button>
            </div>
          </div>
        </div>
      ) : null}
