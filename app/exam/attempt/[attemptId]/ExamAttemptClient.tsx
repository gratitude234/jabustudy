  ...
  const flushPending = useCallback(async () => {
    ...
    const task = (async () => {
      ...
      while (Object.keys(pendingRef.current).length > 0) {
        ...
        setSyncState(pending.revision > 1 ? "retrying" : "saving");
        setSaveError(null);
    ...
    <div className="min-h-dvh bg-background pb-24 text-foreground lg:pb-8">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
        <div className="mx-auto flex min-h-16 max-w-7xl items-center gap-3 px-4 md:px-6">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-black">{attempt.courseCode} · Question {index + 1}/{attempt.questions.length}</p>
            <p className={cn("mt-0.5 inline-flex items-center gap-1.5 text-[11px] font-bold", status.tone)} role="status" aria-live="polite">
              <StatusIcon className={cn("h-3.5 w-3.5", (syncState === "saving" || syncState === "retrying") && online && "animate-spin")} aria-hidden="true" /> {status.label}
            </p>
          </div>
          <div className={cn("inline-flex min-h-10 items-center gap-2 rounded-xl border px-3 font-mono text-sm font-black tabular-nums", criticalTime ? "border-rose-400/40 bg-rose-500/10 text-rose-600 dark:text-rose-300" : lowTime ? "border-amber-400/40 bg-amber-500/10 text-amber-700 dark:text-amber-300" : "border-border bg-card")}>
            <Clock3 className="h-4 w-4" aria-hidden="true" /> {msToClock(remainingMs ?? 0)}
          </div>
          <button type="button" onClick={() => setPaletteOpen(true)} className="hidden min-h-10 items-center gap-2 rounded-xl border border-border bg-card px-3 text-xs font-black sm:inline-flex lg:hidden"><Grid3X3 className="h-4 w-4" aria-hidden="true" /> Review</button>
        </div>
        <div className="h-1 bg-secondary"><div className="h-full bg-primary transition-[width]" style={{ width: `${((index + 1) / attempt.questions.length) * 100}%` }} /></div>
      </header>

      <div className="mx-auto grid max-w-7xl gap-5 px-4 py-5 md:px-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        <main className="min-w-0">
          ...
          <section className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-6 md:p-7">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-primary">Question {index + 1}</p>
            <h1 ref={headingRef} tabIndex={-1} className="scroll-mt-28 pt-3 text-lg font-bold leading-8 outline-none sm:text-xl">{current.prompt}</h1>

            <div className="mt-6 space-y-3" role="radiogroup" aria-label={`Answer options for question ${index + 1}`}>
              {current.options.map((option, optionIndex) => {
                ...
                return (
                  <button
                    ...
                    className={cn("flex min-h-14 w-full items-start gap-3 rounded-xl border p-3.5 text-left transition focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:opacity-60 sm:p-4", selected ? "border-primary bg-primary/[0.08]" : "border-border bg-background hover:border-primary/35 hover:bg-secondary/40")}
                  >
                    ...
                    {selected ? <Check className="mt-1.5 h-5 w-5 shrink-0 text-primary" aria-hidden="true" /> : null}
                  </button>
                );
              })}
            </div>

            <button type="button" disabled={submitting || timeExpired} onClick={() => queueResponse(current.id, { flagged: !currentResponse.flagged })} className={cn("mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl border px-3.5 text-sm font-black disabled:opacity-50", currentResponse.flagged ? "border-amber-300/60 bg-amber-100/50 text-amber-800 dark:bg-amber-950/35 dark:text-amber-300" : "border-border text-muted-foreground hover:bg-secondary hover:text-foreground")}><Flag className="h-4 w-4" aria-hidden="true" /> {currentResponse.flagged ? "Flagged for review" : "Flag for review"}</button>
          </section>
          ...
          <div className="mt-4 hidden items-center justify-between gap-3 lg:flex">
            ...
            <button type="button" disabled={index >= attempt.questions.length - 1 || submitting} onClick={() => setIndex((value) => Math.min(attempt.questions.length - 1, value + 1))} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-primary px-5 text-sm font-black text-primary-foreground disabled:opacity-35">Next <ArrowRight className="h-4 w-4" aria-hidden="true" /></button>
          </div>
        </main>

        <aside className="hidden lg:block">
          <div className="sticky top-24 rounded-2xl border border-border bg-card p-4 shadow-sm">
            <div className="flex items-center justify-between"><p className="text-sm font-black">Review questions</p><span className="text-xs font-bold text-muted-foreground">{answered}/{attempt.questions.length}</span></div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 px-3 pb-[max(0.65rem,env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl lg:hidden" aria-label="Exam navigation">
        <div className="mx-auto grid max-w-lg grid-cols-3 gap-2">
          <button type="button" disabled={index === 0 || submitting} onClick={() => setIndex((value) => Math.max(0, value - 1))} className="inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl border border-border bg-card text-xs font-black disabled:opacity-35"><ArrowLeft className="h-4 w-4" aria-hidden="true" /> Previous</button>
          <button type="button" onClick={() => setPaletteOpen(true)} disabled={submitting} className="inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl border border-primary/25 bg-primary/[0.08] text-xs font-black text-primary"><Grid3X3 className="h-4 w-4" aria-hidden="true" /> Review</button>
          <button type="button" disabled={index >= attempt.questions.length - 1 || submitting} onClick={() => setIndex((value) => Math.min(attempt.questions.length - 1, value + 1))} className="inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl bg-primary text-xs font-black text-primary-foreground disabled:opacity-35">Next <ArrowRight className="h-4 w-4" aria-hidden="true" /></button>
        </div>
      </nav>
      ...
      {paletteOpen ? (
        <div className="fixed inset-0 z-50 flex items-end bg-zinc-950/60 backdrop-blur-sm lg:hidden" role="dialog" aria-modal="true" aria-labelledby="question-review-title">
          <div className="max-h-[min(86dvh,720px)] w-full overflow-y-auto rounded-t-2xl border border-border bg-background px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 shadow-2xl">
            <div className="mx-auto max-w-lg">
              <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-primary">Question review</p><h2 id="question-review-title" className="mt-0.5 text-xl font-black">{answered} answered · {unanswered} unanswered</h2></div><button type="button" onClick={() => setPaletteOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-card" aria-label="Close question review"><X className="h-4 w-4" /></button></div>
              ...
              <PaletteLegend />
              <button type="button" onClick={() => { setPaletteOpen(false); setSubmitDialogOpen(true); }} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white"><Send className="h-4 w-4" aria-hidden="true" /> Review & submit</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {submitDialogOpen ? (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-zinc-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="submit-exam-title">
          <div className="w-full max-w-md rounded-2xl border border-border bg-background p-5 shadow-2xl sm:p-6">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-primary">Final check</p><h2 id="submit-exam-title" className="mt-1 text-2xl font-black">Submit this exam?</h2></div>{!timeExpired ? <button type="button" onClick={() => setSubmitDialogOpen(false)} className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-card" aria-label="Close submission dialog"><X className="h-4 w-4" /></button> : null}</div>
            ...
            <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              {!timeExpired ? <button type="button" onClick={() => setSubmitDialogOpen(false)} className="min-h-11 rounded-xl border border-border px-4 text-sm font-black">Keep checking</button> : null}
              <button type="button" onClick={() => void submitAttempt(timeExpired ? "timeup" : "manual")} disabled={submitting} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-sm font-black text-white disabled:opacity-60">{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} {submitting ? "Submitting…" : timeExpired ? "Retry submission" : "Submit exam"}</button>
            </div>
          </div>
        </div>
      ) : null}
