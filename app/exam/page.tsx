...
export default async function ExamSprintPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <section className="relative isolate overflow-hidden rounded-[1.5rem] bg-[#21164f] px-4 pb-0 pt-4 text-white shadow-[0_18px_50px_rgba(33,22,79,0.18)] sm:px-7 sm:pt-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {user ? (
            catalog.access.active ? (
              <span className="inline-flex min-h-8 items-center gap-1.5 text-[11px] font-black text-emerald-200">
        ...
        <div className="max-w-2xl pb-4 pt-3 sm:pb-6 sm:pt-5">
          <h1 className="max-w-xl text-[1.75rem] font-black leading-[1.08] tracking-[-0.035em] sm:text-5xl">
          ...
          <p className="mt-2 max-w-xl text-[13px] font-medium leading-5 text-violet-100/75 sm:mt-3 sm:text-base sm:leading-6">
            Choose a ready course, answer under a real timer, then review exactly where you lost marks.
          </p>
          <a href="#courses" className="mt-4 inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-white px-4 text-xs font-black text-[#21164f] no-underline shadow-sm sm:min-h-11 sm:text-sm">
            Choose a course <ArrowDown className="h-4 w-4" aria-hidden="true" />
          </a>
        </div>

        <div className="grid grid-cols-3 divide-x divide-white/10 border-t border-white/10">
          <div className="py-2.5 pr-3 sm:py-3.5">
            <p className="text-base font-black tabular-nums sm:text-lg">{available}</p>
          ...
          <div className="px-3 py-2.5 sm:px-5 sm:py-3.5">
            <p className="text-base font-black tabular-nums sm:text-lg">{mockQuestions}</p>
            <p className="mt-0.5 text-[10px] font-bold leading-4 text-violet-200/70">Questions / mock</p>
          </div>
          <div className="py-2.5 pl-3 sm:py-3.5 sm:pl-5">
            <p className="text-base font-black tabular-nums sm:text-lg">{diagnosticQuestions}</p>
      ...
      <details className="group overflow-hidden rounded-2xl border border-border bg-card">
        <summary className="flex min-h-16 cursor-pointer list-none items-center gap-3 px-4 py-3.5 marker:hidden sm:px-5">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><ShieldCheck className="h-[18px] w-[18px]" aria-hidden="true" /></span>
          <span className="min-w-0 flex-1"><span className="block text-sm font-black">How Exam Sprint works</span><span className="mt-0.5 block text-xs text-muted-foreground">Three quick things to know before you begin</span></span>
        ...
        <div className="border-t border-border">
          {[
            ...
            <article key={title} className={`flex gap-3 px-4 py-4 sm:px-5 ${index > 0 ? "border-t border-border" : ""}`}>
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><Icon className="h-4 w-4" aria-hidden="true" /></span>
              <div className="min-w-0 flex-1"><h3 className="text-sm font-extrabold">{title}</h3><p className="mt-1 text-sm leading-5 text-muted-foreground">{body}</p></div>
            </article>
          ))}
        </div>
      </details>
