...
import { rejectBillingOrder } from "@/lib/studyBilling";
...
export async function POST(
  ...
  try {
    ...
    if (!isSuper) return jsonError("Only main Study Admins can reject billing.", 403, "STUDY_ADMIN_REQUIRED");
    const body = await req.json().catch(() => null) as { note?: unknown } | null;
    const order = await rejectBillingOrder(id, userId, body?.note);
