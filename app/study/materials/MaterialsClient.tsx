...
function MaterialCard({
  ...
  return (
    <div className={cn("overflow-hidden border border-border bg-card shadow-sm transition hover:border-border hover:shadow-md", ts.border)}>
      ...
      <div className="flex items-center gap-2 border-t border-border px-4 py-2 text-[11px] text-muted-brand">
        <span className="shrink-0">{formatWhen(m.created_at)}</span>
        <span className="shrink-0">·</span>
        <span className="min-w-0 flex-1 truncate">Uploaded by {uploaderName}</span>
        {(m.up_votes ?? 0) > 0 && (
