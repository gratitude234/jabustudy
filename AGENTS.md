...
GEMINI_STUDY_API_KEYS=       # Optional comma-separated key pool for explanations, grading, summaries, chat, study plans
GEMINI_GENERATION_API_KEYS=  # Optional comma-separated key pool for material/course/rep question generation
...
NEXT_PUBLIC_WHATSAPP_COMMUNITY_URL= # Gratitude Builds / Exam Sprint WhatsApp Channel or community invite; also a legacy Study Hub fallback
...
`lib/ai/index.ts` is the provider-aware server-only AI wrapper. Production currently uses Gemini by default (`AI_PROVIDER=gemini`) with Bedrock hard-disabled unless `AI_ENABLE_BEDROCK=true`. `lib/ai/gemini.ts` routes by model role: `generation` and `document` use Gemini 2.5 Flash, while `fast` uses Gemini 2.5 Flash-Lite. Gemini key selection is workload-aware: normal study/practice calls use `GEMINI_STUDY_API_KEYS` first, heavy question-generation calls use `GEMINI_GENERATION_API_KEYS` first, and both fall back to `GEMINI_API_KEYS` / `GEMINI_API_KEY`. Gemini skips a key temporarily after quota 429; quota errors fail fast once all keys in that workload pool are exhausted. Transient network/server failures still retry/fallback where configured. `lib/ai/bedrock.ts` remains available for later reactivation via env. **Never import AI wrappers in client components.**
